...
int main(int argc, char *argv[])
{
  ...
  // unique_ptr<Solver> solv_lor;
   //solv_lor.reset(new LORSolver<HypreBoomerAMG>(*a, ess_tdof_list));
    
   LORSolver<HypreSmoother> solv_lor(*a, ess_tdof_list);
   solv_lor.GetSolver().SetType(HypreSmoother::GS);
 ...
 //  cg.SetPreconditioner(solv_lor);
   ...
   double fnorm = cg.GetFinalNorm();
    ...
    std::cout << "Solved!" << fnorm << std::endl;
